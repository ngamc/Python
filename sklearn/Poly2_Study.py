...
This is to use sklearn to solve 2 degree polynomial problem
...
#    rm = r/max_number                  # normalize x
    ...
    y_train_list.append(rm*m + rm**2*n + b + noise)
...
#    rm = r/max_number                  # normalize x
    ...
    noise = np.random.normal(mean, std)
    y_test_list.append(rm*m + rm**2*n + b + noise)
...
xx = np.linspace(0, max_number, 100) 
xx_quadratic = quadratic_featurizer.transform(xx.reshape(xx.shape[0], 1)) 
yy_quadratic = regressor_quadratic.predict(xx_quadratic) 

plt.axis([0, max_number, 0, (max_number*max_number*n+ max_number*m+b)]) 
